...
static int buffer_delete(Buffer* buf) {
	if (buf->column >= buf->lines[buf->line].size) {
		if (buf->line + 1 >= buf->size) return 0;
		u64 oll = buf->lines[buf->line].size;
		for (u64 i = 0; i < buf->lines[buf->line + 1].size; i++) {
			line_insert(&buf->lines[buf->line], buf->lines[buf->line + 1].text[i], oll + i);
		}
	...
	if (buf->line >= buf->size) return 0;

	line_remove(&buf->lines[buf->line], buf->column);
	return 0;
}
